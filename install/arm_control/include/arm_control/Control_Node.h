...
namespace motionplaning::resolve
{
  ...
  private:
    ...
    // 位姿参数（原子变量保证线程安全）
    std::atomic<double> default_x_, default_y_, default_z_;
    std::atomic<double> default_roll_, default_pitch_, default_yaw_;
    // 轨迹状态
    std::atomic<double> move_time_{2.0};
    double speed_ = 100;                     // 速度
    std::atomic<int> traj_step_{0};          // 当前走到第几步
    std::atomic<int> traj_total_{0};         // 总步数
    Eigen::Matrix<double, 6, 1> traj_start_; // 起始角
    Eigen::Matrix<double, 6, 1> traj_delta_; // 每一步增量
    ...
    std::shared_ptr<rclcpp::ParameterEventHandler> param_cb_handler_;
